...
CONSONANTS_CYRILLIC = CONSONANTS_REGULAR + [CONSONANT_SH_hard, CONSONANT_SH_soft]
CONSONANTS_EXTCYRILLIC = (
    CONSONANTS_REGULAR
...
VOWELS_CYRILLIC = (
    VOWELS_HARD[0:1] + VOWELS_SOFT[0:3] + VOWELS_HARD[1:5] + VOWELS_SOFT[3:5]
)
EASY_VOWELS_CYRILLIC = VOWELS_HARD[0:1] + VOWELS_SOFT[0:3:2] + VOWELS_HARD[1:3]
...
class WordGenerator:
    ...
    def __init__(
        self, consonants=CONSONANTS_EXTCYRILLIC, vowels=VOWELS_CYRILLIC, length=4
    ):
...
def main():
    ...
    parser = ArgumentParser(
        description="Word id generator", formatter_class=ArgumentDefaultsHelpFormatter
    )
    parser.add_argument(
        "-l", "--length", type=int, default=4, help="number of syllables in the word"
    )
    parser.add_argument(
        "-n",
        "--number",
    ...
    parser.add_argument(
        "-c", "--count", action="store_true", help="print number of possible words"
    )
    parser.add_argument(
        "-s", "--save", action="store_true", help="remember that the word was generated"
    )
    parser.add_argument(
        "-u",
        "--unique",
    ...
    word_generator = WordGenerator(
        length=args.length, consonants=consonants, vowels=vowels
    )
    config_name = "ба" + str(args.length) + ".txt"
    config_path = os.path.join(
        os.path.expanduser("~"), ".config", CONFIG_FOLDER, config_name
    )
